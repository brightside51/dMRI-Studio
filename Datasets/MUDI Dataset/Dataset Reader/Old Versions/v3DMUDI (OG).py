...
class v3DMUDI(Dataset):
    ...
    def __init__(
        ...
        # Label Handling Definitions
        self.patient_id = settings.patient_id
        self.gradient_coord = settings.gradient_coord
        self.num_labels = 7                                         # Initial Number of Labels
        if not settings.patient_id: self.num_labels -= 1            # Exclusion of Patiend ID
        if not settings.gradient_coord: self.num_labels -= 1        # Conversion of Gradient Coordinates to Angles
        if settings.label_norm:                                     # Control Boolean Value for the Normalization of Labels
    ...
    def get_patient(
        ...
        del pMask, available_memory, mask_size, file_size
        pX = np.transpose(pX, (3, 0, 1, 2))                                                         # Full Patient Data Reshapping
        return pX
    ...
    def split_patient(
        ...
        if self.gradient_coord: py = py.drop(columns = ['Gradient theta', 'Gradient phi'])          # Choosing of Gradient Orientation
        else: py = py.drop(columns = ['Gradient x', 'Gradient y', 'Gradient z'])                    # from 3D Cartesian or Polar Referential
        assert(py.shape[1] == self.num_labels), "ERROR: Labels wrongly Deleted!"
...
if __name__ == '__main__':
    ...
    patient_number: int = 0; data = mudi.get_patient(patient_number)
