...
class ConcreteAutoencoder(pl.LightningModule):
    def __init__(
        ...
        #self.pred_sig_save = torch.zeros(136799,1344,dtype=float,requires_grad=False)
    ...
    def on_epoch_end(self) -> None:
        ...
        """if mean_max > 0.998:
            np.savetxt('/cubric/data/sapap9/runs_indices_results/pred_sig14_zebra_seed_xx.txt', self.pred_sig_save.cpu().detach().numpy())"""
        #np.savetxt('/cubric/data/sapap9/runs_indices_results/pred_sig14_zebra_seed_xx.txt', self.pred_sig_save.cpu().detach().numpy())
    ...
    def _shared_eval(
        ...
        """if prefix=="val":
            ...
            #self.pred_sig_save[batch_idx*batch_size:(batch_idx+1)*batch_size,:] = decoded#.cpu().detach().numpy()
            self.pred_sig_save[batch_idx*batch_size:(batch_idx+1)*batch_size,:] = batch#.cpu().detach().numpy()"""
    ...
    def get_indices(self):
        ...
        #print(val)
        #np.savetxt('/home/sapap9/PythonCode/MUDI/runs/textfiles/K=500_epoch=2000_test_subj_dec.txt', np.array(val, dtype=int), fmt='%d')
        return val
