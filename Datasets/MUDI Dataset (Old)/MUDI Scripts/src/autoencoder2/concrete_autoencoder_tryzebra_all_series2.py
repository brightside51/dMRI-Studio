...
class qmrizebra(nn.Module):
    ...
    def getsignals(self, x):
        ...
        #theta = torch.atan2(torch.sqrt(torch.square(mriseq[0,:])+ torch.square(mriseq[1,:])),mriseq[2,:])
        #phi = torch.atan2(mriseq[1,:],mriseq[0,:])#+np.pi
        x1 = mriseq[0,:]
        x2 = mriseq[1,:]
        x3 = mriseq[2,:]
        bval = mriseq[3,:]
...
class ConcreteAutoencoder(pl.LightningModule):
    ...
    def forward(self, x: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        #decoded, decoded2 = self.qmrizebra(x[:,ind_mudi])
