...
# clNN Model Training, Validation & Testing Script Class
class LitclNN(pl.LightningModule):
    ...
    def __init__(
        ...
        self.model_filepath = Path(f"{self.settings.save_folderpath}/V{self.settings.model_version}/clNN (V{self.settings.model_version}).pth")
        if self.model_filepath.exists():

            # Checkpoint Fixing (due to the use of nn.DataParallel)
            print(f"DOWNLOADING Conditional Linear Neural Network (Version {self.settings.model_version})")
    ...
    def reconstruct(
        ...
        h, pX_fake = self.model(torch.Tensor(np.array(self.pX)[self.data.idx_train.astype(int), :]).T)
        ...
        loss = self.criterion(torch.Tensor(pX_fake), torch.Tensor(self.pX_real))

        # --------------------------------------------------------------------------------------------

        # Original Training Example Image Subplot
        figure = plt.figure(num_epochs, figsize = (60, 60))
        patient_id = self.data.patient_info['Patient'].iloc[self.sel_patient]
    ...
    def on_train_start(self):
        ...
        # Dataset Access for Example Patient
        self.sel_patient = 0
        self.data = v1DMUDI.load(   self.settings.data_folderpath,
                                    self.settings.data_version)
        self.pX, self.pMask = self.data.get_patient(self.sel_patient)
        self.pX_real = unmask(self.pX, self.pMask)
        self.pX_real = self.pX_real.get_fdata().T
    ...
    def training_step(self, batch, batch_idx):
        ...
        # Data Handling
        X_train_batch, X_val_batch = batch; reg = 0
        ...
        h_batch, X_fake_batch = self.model(X_train_batch)
        loss = self.criterion(X_fake_batch, X_batch)

        # Regularization Term
        if self.settings.reg_lambda > 0:
            reg = self.model.module.encoder.regularization()
            loss = loss + (self.settings.reg_lambda * reg)

        # Backwards Propagation
        self.optimizer.zero_grad()
        loss.backward()
        self.optimizer.step()
        del X_batch, X_train_batch, X_val_batch, X_fake_batch, h_batch
        return {'loss': loss, 'regularization': reg}
    ...
    def on_train_batch_end(self, loss, batch, batch_idx):
        ...
        self.train_reg = self.train_reg + loss['regularization']
    ...
    def on_train_epoch_end(self):
        ...
        mean_max = self.model.module.encoder.calc_mean_max()
        temp = self.model.module.encoder.update_temp(num_epochs, self.trainer.max_epochs)
        train_plot, train_recon_loss = self.reconstruct(sel_params = 0, sel_slice = 25)
        ...
        self.train_logger.experiment.add_scalar("Regularization Term", self.train_reg, num_epochs)
        self.train_logger.experiment.add_scalar("Mean Max", mean_max, num_epochs)
        self.train_logger.experiment.add_scalar("Softmax Temperature", temp, num_epochs)
